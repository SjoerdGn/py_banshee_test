# -*- coding: utf-8 -*-
"""
Created on Mon Mar  1 15:23:35 2021
@author: Paul

Modified on Wed Jul 14 04:18:42 2021
@author: mmendozalugo
...
def gaussian_distance(R, DATA, SampleSize_1=1000,
                      SampleSize_2=1000, M=1000, Plot=0, Type='H',fig_name=''):
    '''
    GAUSSIAN_DISTANCE Measure the distance between Gaussian densities

    D_ERC,B_ERC,D_BNRC,B_BNRC = gaussian_distance(R, DATA) calculates 
    the distance between the empirical (ERC) and Bayesian Network's (BNRC) 
    rank correlation matrices and the empirical normal rank correlation 
    matrix (NRC) based on DATA. The resulting d-calibration score D and 
    quantile ranges B of determinants of the empirical normal and Bayesian 
    Network's distributions can be used to verify to what extent the 
    assumption of joint normal copula is valid.
    
    INPUT. The required inputs are as follows:

      R               A matrix generated using bn_rankcorr function.
      DATA            A matrix containing data for quantifying the NPBN.
    ...
    OUTPUT. The function returns the following variable:

      D_ERC           A numeric value of the d-calibration score for the 
                      empirical rank correlation matrix of DATA
      D_BNRC          A numeric value of the d-calibration score for the 
                      Bayesian Network rank correlation matrix R.
      B_ERC           Quantile range (5th and 95th percentile) of the
                      distribution of the determinant of the empirical
                      distribution of DATA transformed to standard normal.
      B_BNRC          Quantile range (5th and 95th percentile) of the
                      distribution of the determinant of the empirical
    ...
    OPTIONS. D_ERC,B_ERC,D_BNRC,B_BNRC = gaussian_distance(R,DATA,SAMPLESIZE,PLOT,TYPE) 
    has the following options:

      SAMPLESIZE 1    The number of samples to be drawn in
                      the resampling of the distributions in the test
                      d-Cal(ERC,NRC). 1000 is the default.
      SAMPLESIZE 2    The number of samples to be drawn in
                      the resampling of the distributions in the test
                      d-Cal(NRC,BNRC). 1000 is the default.
      M               Number of iterations of calculating the d-calibration
                      scores to compute the confidence interval
                      of the determinant of the sampled random distribution.
                      1000 is the default.
      PLOT            A plot highlighting the d-calibration scores can be
                      displayed. The options are:
                      0 - do not create a plot (default)
                      1 - create a plot.
      TYPE            Type of measure used to calculate the distance.
    ...
    if Plot==1:
        ...
        fig, ax = plt.subplots(figsize=(10, 10), sharex=False, sharey=False, ncols=2, nrows=1)
        ax[0].step(np.array(x),np.array(f),color='k')
        ax[0].plot([D_ERC, D_ERC],[f[1],1],'r',linewidth=2)
        #ax[0].set_xlim([min(D_ERC-0.05,x[0]),1])
        ...
        #ax[1].set_xlim([min(D_BNRC-0.05,x1[0]),1])
    ...
    if D_ERC > B_ERC[0] and D_ERC < B_ERC[1]:
        print('SUCCESS: The d-Cal of the empirical rank correlation matrix (ERC) fall between the confidence intervals of the d-Cal of the normal rank correlation matrix (NRC)\n')
    else:
        print('FAILURE: The d-Cal of the empirical rank correlation matrix (ERC) is out of the confidence intervals of the d-Cal of the normal rank correlation matrix (NRC)\n')
    
    
    if D_BNRC > B_BNRC[0] and D_BNRC < B_BNRC[1]:
        print('SUCCESS: The d-Cal of the normal rank correlation matrix (NRC) fall between the confidence intervals of the d-Cal of the BN rank correlation matrix (BNRC)\n')
    else:
        print('FAILURE: The d-Cal of the normal rank correlation matrix (NRC) is out of the confidence intervals of the d-Cal of the BN rank correlation matrix (BNRC)\n')
...
def test_distance(Sigma1, Sigma2, Type, Nvar):
    # Sigma1: correlation matrix of the first distribution
    # Sigma2: correlation matrix of the second distribution
    # Type: type of distance metric   
    # Nvar: the number of variables in DATA
    
    # D: distance between matrices
