...
def gaussian_distance(
    ...
    # quantile ranges
    q = [0.05, 0.95]  # defining the quantile range
    ...
    if Plot == 1:
        ...
        fig, ax = plt.subplots(
            figsize=(10, 10), sharex=False, sharey=False, ncols=2, nrows=1
        )
        ...
        ax[0].grid(which="both", ls="--", linewidth=0.5)
        ...
        ax[1].grid(which="both", ls="--", linewidth=0.5)
...
def test_distance(Sigma1, Sigma2, Type, Nvar):
    ...
    if Type == "H":
        ...
        a = (
            np.linalg.det(Sigma1) ** (1 / 4) * np.linalg.det(Sigma2) ** (1 / 4)
        ) / np.linalg.det((1 / 2) * Sigma1 + (1 / 2) * Sigma2) ** (1 / 2)
        b = np.exp(
            -(1 / 8)
            * (m1 - m2).reshape(1, -1)
            @ np.linalg.inv((1 / 2) * Sigma1 + (1 / 2) * Sigma2)
            @ (m1 - m2)
        )
        # equation proper
        D = (1 - (a * b)) ** (1 / 2)
        ...
        D = (1 / 2) * (m1 - m2).reshape(1, -1) @ (
            np.linalg.inv(Sigma1) + np.linalg.inv(Sigma2)
        ) @ (m1 - m2) + (1 / 2) * np.trace(
            np.linalg.inv(Sigma1) * Sigma2
            + Sigma1 * np.linalg.inv(Sigma2)
            - 2 * np.identity(Sigma1.shape[0])
        )
    elif Type == "B":
        # Bhattacharyya distance
        D = (1 / 8) * (m1 - m2).reshape(1, -1) @ np.linalg.inv(
            (1 / 2) * Sigma1 + (1 / 2) * Sigma2
        ) @ (m1 - m2) + (1 / 2) * np.log(
            (np.linalg.det((1 / 2) * Sigma1 + (1 / 2) * Sigma2))
            / (
                (np.linalg.det(Sigma1)) ** (1 / 2)
                * (np.linalg.det(Sigma2)) ** (1 / 2)
            )
        )
    elif Type == "G":
        # G distance (Abou Moustafa et al. 2010)
        L = eigh(Sigma1, Sigma2, eigvals_only=True)
        D = (m1 - m2).reshape(1, -1) @ np.linalg.inv(
            (1 / 2) * Sigma1 + (1 / 2) * Sigma2
        ) @ (m1 - m2) + (sum((np.square(np.log(L))))) ** (1 / 2)
    else:
        # displaying error if no valid distance type was inserted
