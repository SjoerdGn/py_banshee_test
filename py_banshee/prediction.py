...
def inference(Nodes, Values, R, DATA, Output='full', 
    ...
    distributions : list
        ...
        example: ['norm','genextreme','norm']	(Continous Statistical functions of scipy.stats)
    parameters : 
...
def make_dist(distributions,parameters):
    ...
    ----------
    distributions : list
        A list of the names of the distribtions fro each node 
...
def conditional_margins_hist(F,empirical_data,DATA,names,condition_nodes,distributions=[],parameters=[]):  
    '''
    conditional_margins_hist shows the histogram comparison plots of th conditional and uncoditnional samples
    ...
    empirical_data : int
        ...
        distributions=[], parameters=[]
        0 = Nodes are parametric distributions, no empirical observations available
        if 0 DATA should be empty i.e. DATA = []
    DATA : pandas.core.frame.DataFrame
        A matrix containing data for quantifying the NPBN.
        Data for each node need to be located in columns in
        the same order as specified in R. The number of 
