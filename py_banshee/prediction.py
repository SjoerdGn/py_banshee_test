...
Modified on Wed Sep 22 21:40:21 2021
@author: Paul
"""
    ...
    for j in range(n_values):
        ...
        if empirical_data == 1:
            for i in range(len(Nodes)):
                ...
                # columns in DATA.
                i_nodes = Nodes[i]
                x_int = [x[i_nodes][0] - (x[i_nodes][1] - x[i_nodes][0])] + x[
                    i_nodes
                ][1:]
                y_int = [0] + f[i_nodes][1:]
            ...
            for i in range(len(Nodes)):
                NormalCond[i] = norm.ppf(
                    dists[i].cdf(Values[j, i], *params[i])
                )

        # Calculating the parameters of the conditional normal distribution
        # (auxiliary function 2)
        M_c, S_c = ConditionalNormal(
            np.zeros(n_nodes), rpearson, Nodes, NormalCond
        )
        ...
        for i in range(nr_remaining_nodes):
            if empirical_data == 1:
                F0[i] = inv_empirical(
                    norm.cdf(norm_samples[:, i]),
                    [f[remaining_nodes[i]], x[remaining_nodes[i]]],
                    Interp,
                )
            else:
                F0[i] = dists[remaining_nodes[i]].ppf(
                    norm.cdf(norm_samples[:, i]), *params[remaining_nodes[i]]
                )
...
def ecdf(column):
    ...
    a = sq.sort_index().cumsum() * 1.0 / len(column)
    f = a.tolist()
...
def make_dist(distributions, parameters):
    ...
    dist_all_names = np.array(
        [str(dist_all[i].name) for i in range(len(dist_all))]
    )  # names of all
    dists = [
        dist_all[np.where(dist_all_names == distributions[i])[0][0]]
...
def conditional_margins_hist(F, data, names, condition_nodes):
    ...
    try:
        ...
        for i in range(nr_remaining_nodes):
            ...
            plt.hist(
                ...
                label=[
                    "un-conditionalized\n mean: "
                    + str(round(np.mean(F_uncond[:, i]), 1))
                ],
            )
            plt.hist(
                F_cond[:, i],
        ...
        raise Exception(
            "Check if argument Output in inference is equal to full"
        )
