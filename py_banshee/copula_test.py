# -*- coding: utf-8 -*-
"""
Created on Thu Jan  7 10:51:20 2021
@author: Paul

Modified on Thu Jul 15 00:17:52 2021
@author: mmendozalugo
...
def cvm_statistic(DATA, plot, names, fig_name=''):
    """ cvm_statistic calculates the goodness-of-fit for each pair 
     ...
     INPUT. The required input is as follows:
        ...
        DATA      A matrix containing data to be tested. Each column
                  is one variable.
     ...
     OPTIONS. cvm_statistic(DATA,PLOT,NAMES) has the following 
              optional parameters:
 
        PLOT            A plot highlighting the optimal copula per pair of
                        variables can be displayed. The options are:
                        0 - do not create a plot (default);
                        1 - create a plot.
        NAMES           A cell array containing names of the nodes for the 
                        plot; otherwise, default names are assigned.
    """
    ...
    if plot==1:
        fig, ax = plt.subplots(figsize=(10, 10), sharex=False, sharey=False, ncols=N-1, nrows=N-1)
        x = np.arange(4)
        ...
        for i in range(N-1):
            for j in range(N-1):
                if i>j:
                    ...
                    if i==0:
                        ax[i,j].set_title(names[int(M[count,1])],fontsize=14)
                    if i==j:
                        ax[i,j].set_ylabel(names[int(M[count,0])],fontsize=14)
                    count+=1
...
def CVM(OBS, family):
    """ 
    M:      Cramer-von Mises statistic M
    OBS:    nx2 matrix of observations
    family: string containing the name of the copula family to be tested
    ...
    try:
        parm,C = copulafit(family,Uobs) # (auxiliary function 3)
...
def emp_copula(D):
    ...
    D: nx2 matrix containing observations
    """
...
def copulafit(family,Uobs):
    ...
    family: string containing the name of the copula family to be tested
            (1) 'Gaussian', (2)'Frank', (3)'Clayton', (4)'Gumbel'
    Uobs:   nx2 matrix of observations
    Returns the
    """
    ...
    if family=='Gaussian':
        ...
        #cop = GumbelCopula(dim=dims)
        cop = ArchimedeanCopula(family='gumbel', dim=dims)
    elif family=='Clayton':
        #cop = ClaytonCopula(dim=dims)
        cop = ArchimedeanCopula(family='clayton', dim=dims)
    elif family=='Frank':
        #print('Frank copula not working yet')
        #cop = FrankCopula(dim=dims) 
    ...
    if family=='Gaussian':
        ...
        with HiddenPrints():
            parm = cop.fit(Uobs.values , method="cmle")
    ...
    #return parm,cop.cdf(Uobs) # return correlation and theoretical cdf
    return parm,C # return correlation and theoretical cdf

#--------------------------------------------------------------------------
# 4 - Hidden Print calls of a funtion
#--------------------------------------------------------------------------
class HiddenPrints:
    def __enter__(self):
