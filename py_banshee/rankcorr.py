...
Modified on Sat Sep 18 14:31:30 2021
@author: Paul
...
def bn_rankcorr(parent_cell, data, is_data, plot=0, var_names):
    ...
    if is_data == 1:
        if data.shape[1] != len(parent_cell):
            raise argparse.ArgumentTypeError(
                "Number of data columns does not match the number of parent cells"
            )
    ...
    # (see auxiliary function 3 (PartCorr))
    ...
    for i in range(1, N):
        ...
        for j in seq:
            if is_data == 0:
                ...
                T[counter] = pg.partial_corr(
                    data=norm_data,
                    x=norm_data.columns[SO[i]],
                    y=norm_data.columns[j],
                    covar=list(norm_data.columns[i] for i in cond),
                    method="pearson",
                ).r.values[0]
            s = T[counter]
            ...
            for k in range(len(cond) - 1, -1, -1):
                ...
                shat = (
                    s * np.sqrt((1 - T[k] ** 2) * (1 - (r1) ** 2)) + T[k] * r1
                )
                s = shat
            if np.isnan(s):
                R[SO[i], j] = s
    ...
    R = pearsontorank(R)
    if plot == 1:
        # Plotting the rank correlation matrix (auxiliary function 8)
...
# (different from the standard Pingouin function pg.partial_corr).
# ----------------------------------------------------------------------------
def PartCorr(i, j, cond, R, L, m):
    ...
    # Extracting information on the list L (from previous calculations)
    Lc = L[isort][jsort]
    ...
    # Calculating partial correlation of [(i,j) | (cond[0],cond[1:n])]
...
# ----------------------------------------------------------------------------
# 5 - Adding correlation to list L
# ----------------------------------------------------------------------------
