...
def rank_corr_mat_fig(NamesBN,RBN):
    ...
    '''  
    #adding $$ to names of the varables  
    nam = []
    for i in range(len(NamesBN)):
        nam.append(str(r'$'+NamesBN[i]+'$'))
    ...
    plt.setp(ax.get_xticklabels(), fontsize=12, fontweight= 'black')
    plt.setp(ax.get_yticklabels(), fontsize=12, fontweight= 'black')
    
    #velues of the correlations only if the number of variables is less than 15
    if len(nam)<=10:    
